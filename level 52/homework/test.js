...
const applyFilters = () => {
  ...
  let filtered = manager._allCars;

  if (brand.value) {
    filtered = filtered.filter(car =>
      car.brand.toLowerCase().includes(brand.value.toLowerCase())
    );
  }

  if (minPrice.value) {
    filtered = filtered.filter(car => car.price >= Number(minPrice.value));
  }

  if (maxPrice.value) {
    filtered = filtered.filter(car => car.price <= Number(maxPrice.value));
  }

  manager._cars = filtered;
  renderCars();
};
